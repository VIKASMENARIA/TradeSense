...
export default function MarketDashboard() {
    const sortedByGain = [...stockData].sort((a, b) => b.changePercentage - a.changePercentage);
    const upperCircuit = sortedByGain.slice(0, 20);
    const lowerCircuit = sortedByGain.slice(-20).reverse();
    ...
    const RowItem = ({ stock, type }) => (
        <div style={{
            ...
            <div>{stock.price}</div>
            <div style={{ color: type === 'gain' ? 'var(--success)' : 'var(--danger)' }}>
                {stock.changePercentage > 0 ? '+' : ''}{stock.changePercentage}%
            </div>
            <div style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>{stock.volume}</div>
    ...
    return (
        <section className="animate-fade-in" style={{ margin: '2rem 0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                <h2 style={{ fontSize: '1.5rem', letterSpacing: '1px' }}>MARKET DEPTH <span style={{ fontSize: '0.8rem', color: 'var(--accent)', verticalAlign: 'middle' }}>LIVE</span></h2>
                <div style={{ display: 'flex', gap: '1rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                    <span>VOL: <span style={{ color: 'var(--text-main)' }}>High</span></span>
                    <span>VIX: <span style={{ color: 'var(--danger)' }}>14.2</span></span>
                    <span>PCR: <span style={{ color: 'var(--success)' }}>1.2</span></span>
                </div>
            </div>
            ...
            <div className="market-grid">
                ...
                <div className="glass-card" style={{ padding: '0', overflow: 'hidden' }}>
                    ...
                    <div style={{ maxHeight: '600px', overflowY: 'auto' }} className="custom-scrollbar">
                        {upperCircuit.map(s => <RowItem key={s.symbol} stock={s} type="gain" />)}
                    </div>
                </div>
                ...
                <div className="glass-card" style={{ padding: '0', overflow: 'hidden' }}>
                    ...
                    <div style={{ maxHeight: '600px', overflowY: 'auto' }} className="custom-scrollbar">
                        {lowerCircuit.map(s => <RowItem key={s.symbol} stock={s} type="loss" />)}
                    </div>
                </div>
            </div>
        </section>
